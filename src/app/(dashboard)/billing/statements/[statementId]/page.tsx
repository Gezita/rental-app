...
import {
  deriveStatementWorkflowStep,
  StatementWorkflowSteps,
} from "@/components/statement-workflow-steps";
...
export default async function StatementDetailPage({
  ...
  const previewPdf = generateDraftStatementPdfAction.bind(null, statementId);
  const workflowStep = deriveStatementWorkflowStep(statement);
  ...
  return (
    <div className="space-y-6">
      <PageBackNav parent={{ href: "/billing/statements", label: "Statements" }} />
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-3">
          ...
          <StatementWorkflowSteps current={workflowStep} />
        </div>
      ...
      {query.sent && (
        <FlashAlert clearParams={["sent"]}>
          Statement emailed to {statement.tenant.email}. Status updated to sent.
        </FlashAlert>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          {(statement.status === "draft" || !statement.pdfDocument) && (
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Preview PDF</CardTitle>
                <CardDescription>
                  Generate a PDF from the current draft before sending to the tenant.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form action={previewPdf}>
                  <SubmitButton className="w-full" pendingLabel="Generating PDF…">
                    Preview PDF
                  </SubmitButton>
                </form>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <SendStatementForm
                  ...
                  tenantEmail={statement.tenant.email}
                  tenantName={`${statement.tenant.firstName} ${statement.tenant.lastName}`}
                  totalDueLabel={formatMoney(statement.totalDueCents)}
                />
                <p className="text-xs text-muted">
